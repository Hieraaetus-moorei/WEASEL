...
modelPath = '../tank_detection/yolo_models/best.onnx'
...
# instantiate LED on GPIO 12
led = LED(12)

# instantiate ultrasonic sensor (trigger at GPIO 4, echo at 17)
...
class DriveMode:
    ...
    def run(self, mode,
        ...
        # print('#########', user_steering, user_throttle, pilot_steering, pilot_throttle, '##########')
        #user_steering = 0
        #pilot_steering = 0
        """
        ...
        # define the led-ultrasonic detonator function
        def detonator(distance = ultrasonic.distance):
            if distance < 0.45:
                # global user_steering, user_throttle, pilot_steering, pilot_throttle
                # user_steering, user_throttle, pilot_steering, pilot_throttle = (0, 0, 0, 0)
                led.on()
                print(distance)
                sleep(3)
                led.off()
            else:
                print("detonator_else")

        # Initialise YOLOv8 object detector
        #model_path = '../tank_detection/yolo_models/best.onnx'
       ...
       # except:
            #pass
        def tank_detection_thread(cap, modelPath, user_steering, user_throttle, pilot_steering, pilot_throttle):
            ...
            while cap.isOpened():
                ...
                if not ret:
                    break
                result = model.predict(source = frame, conf = 0.87, visualize = True)
                tank_centre = None
                if result[0]:
                    tank_centre = float(result[0].boxes.xyxy[0, 2] + result[0].boxes.xyxy[0, 0]) / 2
                if tank_centre != None:
                    if tank_centre > 0.45* width and tank_centre < 0.55 * width:
                        user_steering = 0
                    elif tank_centre < width*1 / 6:
                        user_steering = -0.75
                    elif tank_centre < width*2 / 6:
                        user_steering = -0.5
                    elif tank_centre < width*3 / 6:
                        user_steering = -0.25
                    elif tank_centre < width*4 / 6:
                        user_steering = 0.25
                    elif tank_centre < width*5 / 6:
                        user_steering = 0.5
                    elif tank_centre < width*6 / 6:
                        user_steering = 0.75
                    detonator(ultrasonic.distance)
                    print('after detonator')
        ...
        if mode == 'user':
            print("Here")
            (se, mo, us_s, us_t, pi_s, pi_t) = tank_detection_thread(cap, modelPath, user_steering, user_throttle, pilot_steering, pilot_throttle)
            return us_s, us_t
        elif mode == 'local_angle':
            (se, mo, us_s, us_t, pi_s, pi_t) = tank_detection_thread(cap, modelPath, user_steering, user_throttle, pilot_steering, pilot_throttle)
            (user_steering, user_throttle) = (us_s, us_t)
            (pilot_steering, pilot_throttle) = (user_steering, user_throttle)
            print(5)
...
def add_camera(V, cfg, camera_type = 'stereo'):
    """
